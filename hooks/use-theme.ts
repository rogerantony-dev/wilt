...
// Wilt registers no extra themes with Uniwind, so its setTheme only knows the
// three built-ins; PanelUI's brand families are typed here but never used.
...
export function useTheme() {
  ...
  return {
    ...
    setTheme: (name: ThemeName) => Uniwind.setTheme(name as UniwindTheme),
  };
}
...
export function useThemeMode() {
  ...
  return {
    ...
    setFamily: (id: string) => {
      ...
      if (next) Uniwind.setTheme(next[mode] as UniwindTheme);
    },
    /** Set the mode, staying in the current family. */
    setMode: (next: ThemeMode) => Uniwind.setTheme(family[next] as UniwindTheme),
    /** Flip light ↔ dark within the current family. */
    toggleMode: () => Uniwind.setTheme((mode === 'dark' ? family.light : family.dark) as UniwindTheme),
  };
}
